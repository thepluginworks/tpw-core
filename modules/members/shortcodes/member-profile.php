...
function tpw_member_profile_render_profile_section( $member ) {
  ...
  foreach ( $fields as $f ) {
    $key = $f['key'];
    if ( in_array( $key, ['username','password_hash','is_admin','is_committee','is_match_manager','is_noticeboard_admin','is_gallery_admin','is_manage_members','is_volunteer','status'], true ) ) {
